...
CVG001Freqs <- CVG001Freqs[!(CVG001Freqs$Frequency==0),]
...
Days_post <- CVG001Freqs$Days_post_transplant #days post transplant
OTUs<- CVG001Freqs$OTU #OTU
Count <- CVG001Freqs$Frequency #Freqs
ID <- "CVG001"
Genus <- CVG001Freqs$Genus
...
#  1 |  2 |  3 |   50   |
#  1 |  1 |  3 |   25   |
#so the file is OTU001dfm

write.csv(OTU001dfm, file = "OTU001dfm.csv")

#here i remove from the dataframe any OTUs that were either classed as contmainants or were not able to be classified
OTU001dfm <- subset(OTU001dfm, OTU != "OTU_180" & OTU != "OTU_268" & OTU != "OTU_344" & OTU != "OTU_354" & OTU != "OTU_360" & OTU != "OTU_372" & OTU != "OTU_117" & OTU != "OTU_137" & OTU != "OTU_30" & OTU != "OTU_19" & OTU != "OTU_129" & OTU != "OTU_156" & OTU != "OTU_188" & OTU != "OTU_260" & OTU != "OTU_76" & OTU != "OTU_241" & OTU != "OTU_242" & OTU != "OTU_63" & OTU != "OTU_106" & OTU != "OTU_135" & OTU != "OTU_303" & OTU != "OTU_222" & OTU != "OTU_56" & OTU != "OTU_120" & OTU != "OTU_133" & OTU != "OTU_200" & OTU != "OTU_146" & OTU != "OTU_177" & OTU != "OTU_507" & OTU != "OTU_247" & OTU != "OTU_249")

fillseq <- seq(1, length(OTU001dfm$OTU)) #make list of numbers corresponding with number of unique clonotypes

GeneraColours <- tax_data_table$Genus
GeneraColours <- unique(GeneraColours)
GeneraColours <- sort(GeneraColours)

GeneraColours <- as.data.frame(GeneraColours, stringsAsFactors=FALSE)
names(GeneraColours)[1] <- "Genus"

fillseqUniversal <- seq(1, 195) #make list of numbers corresponding with number of unique clonotypes
GeneraColours$fillseqUniversal <- NA
GeneraColours$fillseqUniversal <- fillseqUniversal


OTU001dfmA <- OTU001dfm[order(OTU001dfm$Genus),]
OTU001dfm$Genus <- as.character(OTU001dfm$Genus)
...
temp1<-merge(x=temp1,y=GeneraColours,by="Genus",all.x=TRUE)
...
temp2<-merge(x=temp2,y=GeneraColours,by="Genus",all.x=TRUE)
...
temp3<-merge(x=temp3,y=GeneraColours,by="Genus",all.x=TRUE)
...
temp4<-merge(x=temp4,y=GeneraColours,by="Genus",all.x=TRUE)
...
temp5<-merge(x=temp5,y=GeneraColours,by="Genus",all.x=TRUE)
...
yadjst <- 1 # so we can move around the not shared label
...
stackedShared <- stackedShared[,c(5,6,8)] #make a dataframe with just cdr3seq and fillseq number
...
geom_bar(aes(x = 1, temp5.mod$Count, fill = as.factor(temp5.mod$fillseq), alpha = as.factor(temp5.mod$shared)), stat = 'identity', colour = 'black') +
  ...
  geom_bar(aes(x = 9, temp1.mod$Count, fill = as.factor(temp1.mod$fillseq), alpha = as.factor(temp1.mod$shared)), stat = 'identity', colour = 'black') + 
         ...
         # coord_cartesian(ylim=c(round(minPcntShared-yadjst), 100), expand = T) +
          labs(title = 'Shared Genera CVG001', x = ' ', y = 'Cumulative % of Sample')+
          theme(legend.position="right", legend.title=element_blank(), panel.background = element_blank()) + coord_flip() + guides(alpha = FALSE) + scale_fill_discrete(breaks=c(stackedShared$fillseq), labels = c(stackedShared$Genus)) # + scale_fill_discrete(name = "Genus", labels = c(stackedShared$Genus))
        
ggsave(paste(wd, '/', 'CVG001_', paste(sampleName1, sampleName2, sampleName3, sampleName4, sampleName5, sep = '_'), '_taxa_Genus_updated',  '.pdf', sep = ''), 
...
ggp + geom_text(aes(label = Genus), size = 3, hjust = 0.5, vjust = 3, position = "stack")
```
