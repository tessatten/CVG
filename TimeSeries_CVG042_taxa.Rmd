...
CVG042Freqs <- read.csv(file = "CVG042Freqs.csv", stringsAsFactors = F) #import otu table

CVG042Freqs <- CVG042Freqs[!(CVG042Freqs$Frequency==0),]
...
Days_post <- CVG042Freqs$Days_post_transplant #days post transplant
OTUs<- CVG042Freqs$OTU #OTU
Count <- CVG042Freqs$Frequency #Freqs
ID <- "CVG042"
Genus <- CVG042Freqs$Genus
...
temp3.mod$shared[length(temp3.mod$ID)] <- F
temp4.mod$shared[length(temp4.mod$ID)] <- F #puts the last entry back to false
...
temp4.mod$fillseq[length(temp4.mod$ID)] <- 900000 #puts the last entry back last in fillseq order
...
geom_bar(aes(x = 1, temp5.mod$Count, fill = as.factor(temp5.mod$fillseq), alpha = as.factor(temp5.mod$shared)), stat = 'identity', colour = 'black') +
  ...
  geom_bar(aes(x = 9, temp1.mod$Count, fill = as.factor(temp1.mod$fillseq), alpha = as.factor(temp1.mod$shared)), stat = 'identity', colour = 'black') + 
         ...
         # coord_cartesian(ylim=c(round(minPcntShared-yadjst), 100), expand = T) +
          labs(title = 'Shared taxa CVG042', x = ' ', y = 'Cumulative % of Sample')+
...
ggsave(paste(wd, '/', 'CVG042_', paste(sampleName1, sampleName2, sampleName3, sampleName4, sampleName5, sep = '_'), '_taxa_Genus_large',  '.pdf', sep = ''), 
