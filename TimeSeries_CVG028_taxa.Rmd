...
CVG028Freqs <- read.csv(file = "CVG028.csv", stringsAsFactors = F) #import otu table

CVG028Freqs <- CVG028Freqs[!(CVG028Freqs$Frequency==0),]
...
Days_post <- CVG028Freqs$Days_post_transplant #days post transplant
OTUs<- CVG028Freqs$OTU #OTU
Count <- CVG028Freqs$Frequency #Freqs
ID <- "CVG028"
Genus <- CVG028Freqs$Genus
...
geom_bar(aes(x = 1, temp5.mod$Count, fill = as.factor(temp5.mod$fillseq), alpha = as.factor(temp5.mod$shared)), stat = 'identity', colour = 'black') +
  ...
  geom_bar(aes(x = 9, temp1.mod$Count, fill = as.factor(temp1.mod$fillseq), alpha = as.factor(temp1.mod$shared)), stat = 'identity', colour = 'black') + 
         ...
         # coord_cartesian(ylim=c(round(minPcntShared-yadjst), 100), expand = T) +
          labs(title = 'Shared taxa CVG028', x = ' ', y = 'Cumulative % of Sample')+
...
ggsave(paste(wd, '/', 'CVG028_', paste(sampleName1, sampleName2, sampleName3, sampleName4, sampleName5, sep = '_'), '_taxa_Genus_large',  '.pdf', sep = ''), 
