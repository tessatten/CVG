...
temp2$shared[temp2$OTUs %in% temp3$OTUs | temp2$OTUs %in% temp5$OTUs] <- T  # marks clonotypes that are in temp2 and temp1 as TRUE
temp2$shared[!temp2$OTUs %in% temp3$OTUs & !temp2$OTUs %in% temp5$OTUs] <- F #  marks clonotypes that are in temp2 and NOT in temp1 as FALSE

temp3$shared[temp3$OTUs %in% temp2$OTUs | temp3$OTUs %in% temp5$OTUs] <- T  # marks clonotypes that are in temp2 and temp1 as TRUE
temp3$shared[!temp3$OTUs %in% temp2$OTUs & !temp3$OTUs %in% temp5$OTUs] <- F #  marks clonotypes that are in temp2 and NOT in temp1 as FALSE


temp5$shared[temp5$OTUs %in% temp2$OTUs | temp5$OTUs %in% temp3$OTUs] <- T  # marks clonotypes that are in temp2 and temp1 as TRUE
temp5$shared[!temp5$OTUs %in% temp2$OTUs & !temp5$OTUs %in% temp3$OTUs] <- F # marks clonotypes that are in temp2 and NOT in temp1 as FALSE
...
maxPcntShared <- max(c(100 - sum(temp2.mod$Count[temp2.mod$shared==T]), 100 - sum(temp3.mod$Count[temp3.mod$shared==T]),100 - sum(temp5.mod$Count[temp5.mod$shared==T]))) #calculates highest % of uniques

minPcntShared <- min(c(100 - sum(temp2.mod$Count[temp2.mod$shared==T]), 100 - sum(temp3.mod$Count[temp3.mod$shared==T]), 100 - sum(temp5.mod$Count[temp5.mod$shared==T]))) #calculates lowest % of uniques (gives % of reads that come from clonotypes unique to the sample)
...
intersectC1 <- temp3.mod[temp3.mod$OTUs %in% temp5.mod$OTUs,] # overlap overlap( finds all the seqs in temp1 that also appear in temp 2)
intersectC2 <- temp5.mod[temp5.mod$OTUs %in% temp3.mod$OTUs,] # overlap overlap( finds all the seqs in temp1 that also appear in temp 2)
...
sampleName5 <- paste(unique(temp5.mod$Days_post))
 ...
 #legend adventures
stackedShared <- rbind(temp2.mod, temp3.mod, temp5.mod) #add together shared sequences
...
geom_bar(aes(x = 1, temp5.mod$Count, fill = as.factor(temp5.mod$fillseq), alpha = as.factor(temp5.mod$shared)), stat = 'identity', colour = 'black') +
  ...
  geom_bar(aes(x = 5, temp2.mod$Count, fill = as.factor(temp2.mod$fillseq), alpha = as.factor(temp2.mod$shared)), stat = 'identity', colour = 'black')  + 
          ...
          scale_x_continuous(breaks = c(1, 3, 5), labels = c(sampleName5, sampleName3, sampleName2))+
...
ggsave(paste('CVG017_', paste(sampleName2, sampleName3, sampleName5, sep = '_'), '_taxa_Genus_large',  '.pdf', sep = ''), 
