...
CVG021Freqs <- read.csv(file = "CVG021.csv", stringsAsFactors = F) #import otu table

CVG021Freqs <- CVG021Freqs[!(CVG021Freqs$Frequency==0),]
...
Days_post <- CVG021Freqs$Days_post_transplant #days post transplant
OTUs<- CVG021Freqs$OTU #OTU
Count <- CVG021Freqs$Frequency #Freqs
ID <- "CVG021"
Genus <- CVG021Freqs$Genus
...
geom_line(aes(x = intersectA.mod$x + 1.8, y = intersectA.mod$y, group = intersectA.mod$fillseq), colour = 'grey')+
  ...
  geom_bar(aes(x = 7, temp1.mod$Count, fill = as.factor(temp1.mod$fillseq), alpha = as.factor(temp1.mod$shared)), stat = 'identity', colour = 'black') + 
         ...
         # coord_cartesian(ylim=c(round(minPcntShared-yadjst), 100), expand = T) +
          labs(title = 'Shared taxa CVG021', x = ' ', y = 'Cumulative % of Sample')+
...
ggsave(paste(wd, '/', 'CVG021_', paste(sampleName1, sampleName2, sampleName3, sampleName4, sep = '_'), '_taxa_Genus_large',  '.pdf', sep = ''), 
