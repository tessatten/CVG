...
CVG047Freqs <- read.csv(file = "CVG047.csv", stringsAsFactors = F) #import otu table

CVG047Freqs <- CVG047Freqs[!(CVG047Freqs$Frequency==0),]
...
Days_post <- CVG047Freqs$Days_post_transplant #days post transplant
OTUs<- CVG047Freqs$OTU #OTU
Count <- CVG047Freqs$Frequency #Freqs
ID <- "CVG047"
Genus <- CVG047Freqs$Genus
...
geom_bar(aes(x = 9, temp5.mod$Count, fill = as.factor(temp5.mod$fillseq), alpha = as.factor(temp5.mod$shared)), stat = 'identity', colour = 'black') +
  ...
  geom_bar(aes(x = 17, temp1.mod$Count, fill = as.factor(temp1.mod$fillseq), alpha = as.factor(temp1.mod$shared)), stat = 'identity', colour = 'black') + 
         ...
         # coord_cartesian(ylim=c(round(minPcntShared-yadjst), 100), expand = T) +
          labs(title = 'Shared taxa CVG047', x = ' ', y = 'Cumulative % of Sample')+
...
ggsave(paste(wd, '/', 'CVG047_', paste(sampleName1, sampleName2, sampleName3, sampleName4, sampleName5, sampleName6, sampleName7, sampleName8, sampleName9, sep = '_'), '_taxa_Genus_large',  '.pdf', sep = ''), 
