...
CVG046Freqs <- read.csv(file = "CVG046.csv", stringsAsFactors = F) #import otu table

CVG046Freqs <- CVG046Freqs[!(CVG046Freqs$Frequency==0),]
...
Days_post <- CVG046Freqs$Days_post_transplant #days post transplant
OTUs<- CVG046Freqs$OTU #OTU
Count <- CVG046Freqs$Frequency #Freqs
ID <- "CVG046"
Genus <- CVG046Freqs$Genus
...
geom_line(aes(x = intersectA.mod$x + 1.8, y = intersectA.mod$y, group = intersectA.mod$fillseq), colour = 'grey')+
  ...
  geom_bar(aes(x = 7, temp1.mod$Count, fill = as.factor(temp1.mod$fillseq), alpha = as.factor(temp1.mod$shared)), stat = 'identity', colour = 'black') + 
         ...
         # coord_cartesian(ylim=c(round(minPcntShared-yadjst), 100), expand = T) +
          labs(title = 'Shared taxa CVG046', x = ' ', y = 'Cumulative % of Sample')+
...
ggsave(paste(wd, '/', 'CVG046_', paste(sampleName1, sampleName2, sampleName3, sampleName4, sep = '_'), '_taxa_Genus_large',  '.pdf', sep = ''), 
